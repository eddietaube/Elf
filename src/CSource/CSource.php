...
class CSource {
  ...
  public function GetBreadcrumbFromPath() {
    ...
    $html .= "<li><a href='index.php?p=source'>" . basename($this->baseDir) . "</a>/</li>";
    $path = null;
    foreach($this->breadcrumb as $val) {
      $path .= "$val/";
      $html .= "<li><a href='index.php?p=source&path={$path}'>{$val}</a>/</li>";
    }
    $html .= "</ul>\n";
  ...
  public function ReadCurrentDir() {
    ...
    foreach(glob($this->dir . '/{*,.?*}', GLOB_MARK | GLOB_BRACE) as $val) {
      ...
      $html .= "<li><a href='?p=source&path={$path}'>{$file}</a></li>\n";
    }
    $html .= "</ul>\n";
